...
- **Path parameter validation:** `server_id`, `zone_id`, and zone `subpath` are validated against strict allowlists (`[A-Za-z0-9._-]+` / `[A-Za-z0-9._/-]+`) at the top of each proxy handler. Anything outside that gets a 400 before reaching the upstream, cutting reflected-XSS taint flows at the source.
